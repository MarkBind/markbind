  ...
  return new Promise((resolve, reject) => {
    ...
    fs.readFile(actualFilePath, (err, data) => {
      ...
      if (fileExt === 'md') {
        ...
        const err = new Error(`Unsupported File Extension: '${fileExt}'`);
        reject(err);
      }
    });
  });
...
Parser.prototype._rebaseReference = function (node, foundBase) {
  if (_.isArray(node)) {
    return node.forEach(el => this._rebaseReference(el, foundBase));
  }
  ...
  if (node.attribs[ATTRIB_CWF]) {
    const path = node.attribs[ATTRIB_CWF];
    const newBase = calculateNewBaseUrl(path, this.rootPath, this.baseUrlMap);
    if (newBase) {
    ...
    if (bases.length !== 0) {
      // need to rebase
      const newBase = childrenBase[bases[0]];
