...
Site.prototype.generatePages = function () {
  ...
  return this.buildSiteData().then((frontMatterData) => {
    //console.log(frontMatterData);
    const pages = frontMatterData || [];
...
/**
 * Builds the siteData file that specifies addressable pages and their associated keywords
 */
Site.prototype.buildSiteData = function () {
  const filePaths = walkSync(this.rootPath, { directories: false })
    .filter(currentPath => currentPath.endsWith('.md') && !currentPath.startsWith('_site') && !currentPath.startsWith('_boilerplates'));
  const frontMatterData = [];
  const promises = [];
  //console.log(filePaths);
  filePaths.forEach((currentFile) => {
    promises.push(
      new Promise((resolve, reject) => {
        ...
        markbinder.includeFile(path.resolve(this.rootPath, currentFile), config)
            .then((result) => {
              let $ = cheerio.load(result);
              if ($('frontmatter div').get("0") === undefined) {
                  if ($('frontmatter').get("0") !== undefined) {
                    let data = ($('frontmatter').get("0").children[0].data);
                let matter = "---\n" + data + "---";
                let content = fm(matter);
                //console.log(content);
                if (Object.keys(content.attributes).length !== 0) {
                  content.attributes.src = currentFile;
                  frontMatterData.push(content.attributes);
                }
                resolve();  
                  }
                else resolve();
                ...
                let data = ($('frontmatter div').get("0").children[0].data);
                let matter = "---\n" + data + "---";
                let content = fm(matter);
                //console.log(content);
                if (Object.keys(content.attributes).length !== 0) {
