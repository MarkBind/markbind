...
Site.prototype.collectUserDefinedVariablesMap = function () {
  ...
  this.userDefinedVariablesMap = {};
  const glyphiconsMap = getGlyphiconsMap();

  Object.keys(this.baseUrlMap).forEach((base) => {
    const userDefinedVariables = {};
    Object.assign(userDefinedVariables, glyphiconsMap);
...
Site.prototype.generatePages = function () {
  ...
  const builtFiles = {};
  const pages = this.addressablePages || [];
  ...
  this.pages = pages.map(page => this.createPageData({
    baseUrl,
    faviconUrl,
    pageSrc: page.src,
    title: page.title,
...
Site.prototype.writeSiteData = function () {
  return new Promise((resolve, reject) => {
    ...
    const siteData = { pages: this.pages.map(page => (page.frontMatter)) };
...
Site.prototype._setTimestampVariable = function () {
  const markbindLink = `<a href='https://markbind.github.io/markbind/'>MarkBind ${CLI_VERSION}</a>`;
  const time = new Date().toUTCString();
  const html = `<small>[Generated by ${markbindLink} on ${time}]</small>`;
  Object.keys(this.userDefinedVariablesMap).forEach((base) => {
    this.userDefinedVariablesMap[base].timestamp = html;
  });
  return Promise.resolve();
};
