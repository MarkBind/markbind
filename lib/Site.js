...
Site.prototype.createPageData = function (config) {
  ...
  return new Page({
    ...
    asset: {
      ...
      highlight: path.relative(path.dirname(resultPath), path.join(this.siteAssetsDestPath, 'css', 'github.min.css')),
      markbind: path.relative(path.dirname(resultPath), path.join(this.siteAssetsDestPath, 'css', 'markbind.css')),
...
Site.prototype.collectBaseUrl = function () {
  return new Promise((resolve, reject) => {
    ...
    const candidates = walkSync(this.rootPath, { directories: false }).filter(x => x.endsWith('site.json')).map(x => path.resolve(x));
