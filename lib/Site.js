...
Site.initSite = function (rootPath) {
  ...
  return new Promise((resolve, reject) => {
    fs.accessAsync(configPath)
      .catch(() => fs.writeJsonAsync(configPath, SITE_CONFIG_DEFAULT))
...
Site.prototype.generatePages = function (config) {
  ...
  let processingFiles = [];
  pages.forEach((page) => {
    let newPage = this.createPageData(page.src, page.title, this.pageTemplate);
    processingFiles.push(newPage.generate());
  });
  return new Promise((resolve, reject) => {
