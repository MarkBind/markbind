...
function getIconsMap() {
  ...
  const iconsMap = {};
  // Collate Font Awesome Icons
  fontAwesomeClasses.forEach((fontAwesomeClass) => {
    const faName = fontAwesomeClass.replace(' fa', '').replace(/-/g, '_');
    const faHtml = `<span class='${fontAwesomeClass}'></span>`;
    iconsMap[faName] = faHtml;
  });
  // Collate Glyphicons
  glyphiconsClasses.forEach((glyphiconClass) => {
    const glyphName = glyphiconClass.replace(/-/g, '_');
    const glyphHtml = `<span class='glyphicon ${glyphiconClass}' aria-hidden='true'></span>`;
    iconsMap[glyphName] = glyphHtml;
  });
  return iconsMap;
}
  ...
  return new Page({
    ...
    asset: {
      ...
      fontAwesome: path.relative(path.dirname(resultPath),
                                 path.join(this.siteAssetsDestPath, 'css', 'font-awesome.css')),
      glyphicons: path.relative(path.dirname(resultPath),
                                path.join(this.siteAssetsDestPath, 'css', 'glyphicons.css')),
