...
/**
 * Build all pages and the index of the site
 */
Site.prototype.buildSourceFiles = function () {
  return new Promise((resolve, reject) => {
    this.generatePages()
      ...
      .then(() => fs.writeJsonAsync(path.join(this.rootPath, 'siteData.json'),
                                    { pages: this.frontMatterData }))
      .then(() => logger.info('Site index built'))
      .then(resolve)
      .catch((error) => {
...
Site.prototype.generatePages = function () {
  ...
  const paths = this.addressablePaths || [];
  this.pageModels = paths.map((src) => {
    const index = this.siteConfig.pages.findIndex(currentPage => currentPage.src === src);
    return this.createPageData({
      baseUrl,
      title: this.siteConfig.pages[index] ? this.siteConfig.pages[index].title : '',
      pageSrc: src,
 ...
 */
Site.prototype.collectAddressablePaths = function () {
  const { pages } = this.siteConfig;
  const userGlobs = pages.filter(currentPage => currentPage.glob).map(currentGlob => currentGlob.glob);
  const userPages = pages.filter(currentPage => currentPage.src).map(currentPage => currentPage.src);
  ...
  this.addressablePaths = Array.from(new Set(filePathIndex.concat(userPages)));
};
  ...
  return new Promise((resolve, reject) => {
    Promise.all(processingFiles)
      .then((updatedFrontMatter) => {
        ...
        updatedFrontMatter.forEach((currentData) => {
          const toUpdate = this.frontMatterData.findIndex(frontMatter => frontMatter.src === currentData.src);
          this.frontMatterData[toUpdate] = currentData;
        });
      }).then(() => fs.writeJsonAsync(path.join(this.rootPath, 'siteData.json'),
                                      { pages: this.frontMatterData }))
      .then(() => logger.info('Site index rebuilt'))
      .then(resolve)
      .catch(reject);
  });
};
