  ...
  findRootFolder: (userSpecifiedRoot, siteConfigPath = SITE_CONFIG_NAME) => {
    if (userSpecifiedRoot) {
      ...
      const expectedConfigPath = path.join(
        resolvedUserSpecifiedRoot,
        siteConfigPath,
      );
      if (!fs.existsSync(expectedConfigPath)) {
        throw new Error(
          `Config file not found at user specified root ${resolvedUserSpecifiedRoot}`,
        );
      }
    ...
    const foundConfigPath = findUp.sync(siteConfigPath, {
      cwd: currentWorkingDir,
    });
    if (!foundConfigPath) {
      throw new Error(
        `No config file found in parent directories of ${currentWorkingDir}`,
      );
    }
