...
function formatSiteNav(renderedSiteNav, src) {
  ...
  listItems
    .find(`a[href='{{baseUrl}}/${currentPageHtmlPath}']`)
    .addClass('current');
  ...
  listItems.each(function() {
    ...
    if ($(this).children('a').length) {
      ...
      if (nestedList.length) {
        ...
        nestedList
          .parent()
          .replaceWith(formatSiteNav(nestedList.parent().html(), src));
      }
      ...
      const shouldExpandDropdown =
        dropdownTitle.toString().includes(DROPDOWN_EXPAND_KEYWORD) ||
        Boolean(
          nestedList
            .find(`a[href='{{baseUrl}}/${currentPageHtmlPath}']`)
            .text(),
      ...
      if (shouldExpandDropdown) {
        ...
        const dropdownTitleWithoutKeyword = dropdownTitle
          .toString()
          .replace(expandKeywordRegex, '');
        const rotatedIcon = cheerio
          .load(DROPDOWN_BUTTON_ICON_HTML, { xmlMode: false })('i')
          .addClass('rotate-icon');
        nestedList.wrap(
          '<div class="dropdown-container dropdown-container-open"></div>',
        );
        $(this).prepend(
          '<button class="dropdown-btn dropdown-btn-open">' +
 ...
 */
Page.prototype.collectHeadingsAndKeywordsInContent = function(
  content,
  lastHeading,
  excludeHeadings,
  ...
  $('panel')
    ...
    .each((index, panel) => {
      if (panel.attribs.header) {
        this.collectHeadingsAndKeywordsInContent(
          md.render(panel.attribs.header),
          lastHeading,
          excludeHeadings,
          sourceTraversalStack,
        );
    ...
    .each((index, panel) => {
      const shouldExcludeHeadings =
        excludeHeadings || panel.attribs.expanded === undefined;
      ...
      if (panel.attribs.src) {
        ...
        const includeRelativeToBuildRootDirPath = this.baseUrl
          ...
          : src.substring(1);
        const includeAbsoluteToBuildRootDirPath = path.resolve(
          this.rootPath,
          includeRelativeToBuildRootDirPath,
        );
        const includeRelativeToInnerDirPath = path.relative(
          buildInnerDir,
          includeAbsoluteToBuildRootDirPath,
        );
        const includePath = path.resolve(
          resultInnerDir,
          includeRelativeToInnerDirPath,
        );
        ...
        if (
          childSourceTraversalStack.length >
          CyclicReferenceError.MAX_RECURSIVE_DEPTH
        ) {
...
Page.prototype.addAnchors = function(content) {
  ...
  if (this.headingIndexingLevel > 0) {
    ...
    $('panel[header]').each((i, panel) => {
      const panelHeading = cheerio.load(md.render(panel.attribs.header), {
        xmlMode: false,
      });
      if (panelHeading(headingsSelector).length >= 1) {
        const headingId = $(panelHeading(headingsSelector)[0]).attr('id');
        const anchorIcon = ANCHOR_HTML.replace(/"/g, "'").replace(
          '#',
          `#${headingId}`,
        );
...
Page.prototype.collectFrontMatter = function(includedPage) {
  ...
  if (frontMatter.text().trim()) {
    ...
    this.frontMatter.layout =
      this.layout || this.frontMatter.layout || LAYOUT_DEFAULT_NAME;
  } else {
...
Page.prototype.insertFooter = function(pageData) {
  ...
  // Map variables
  const newBaseUrl =
    calculateNewBaseUrl(this.sourcePath, this.rootPath, this.baseUrlMap) || '';
  const userDefinedVariables = this.userDefinedVariablesMap[
    path.join(this.rootPath, newBaseUrl)
  ];
...
Page.prototype.insertSiteNav = function(pageData) {
  ...
  const siteNavMappedData = nunjucks.renderString(
    siteNavContent,
    userDefinedVariables,
  );
  // Convert to HTML
...
Page.prototype.insertPageNavWrapper = function(pageData) {
  if (this.isPageNavigationSpecifierValid()) {
    const wrappedPageData =
      `<div id="${PAGE_NAV_CONENT_WRAPPER_ID}">\n` +
      `${pageData}\n` +
      '</div>\n';
...
Page.prototype.generatePageNavHeadingHtml = function() {
  ...
  Object.keys(this.navigableHeadings).forEach(key => {
    ...
    const currentHeadingHTML =
      `<a class="nav-link py-1" href="#${key}">` +
      `${this.navigableHeadings[key].text}&#x200E;</a>\n`;
    const nestedHeadingHTML =
      '<nav class="nav nav-pills flex-column my-0"' +
      `style="margin-left: 5%; flex-wrap: nowrap;">\n${currentHeadingHTML}`;

    if (
      headingStack.length === 0 ||
      headingStack[headingStack.length - 1] === currentHeadingLevel
    ) {
      ...
      if (topOfHeadingStack < currentHeadingLevel) {
        ...
        while (
          ...
          topOfHeadingStack > currentHeadingLevel
        ) {
          headingHTML += '</nav>\n';
          headingStack.pop();
  ...
  while (
    ...
    headingStack[headingStack.length - 1] >
      headingStack[headingStack.length - 2]
  ) {
    headingHTML += '</nav>\n';
    headingStack.pop();
  }
...
Page.prototype.collectHeadFiles = function(baseUrl, hostBaseUrl) {
  ...
  if (head) {
    ...
    headFiles = [
      path.join(LAYOUT_FOLDER_PATH, this.frontMatter.layout, LAYOUT_HEAD),
    ];
  }
  headFiles.forEach(headFile => {
    ...
    const headFileMappedData = nunjucks
      .renderString(headFileContent, userDefinedVariables)
      .trim();
    // Split top and bottom contents
    const $ = cheerio.load(headFileMappedData, { xmlMode: false });
    if ($('head-top').length) {
      collectedTopContent.push(
        nunjucks
          .renderString($('head-top').html(), { baseUrl, hostBaseUrl })
          .trim()
          .replace(/\n\s*\n/g, '\n')
          .replace(/\n/g, '\n    '),
      );
    ...
    collectedBottomContent.push(
      nunjucks
        .renderString($.html(), { baseUrl, hostBaseUrl })
        .trim()
        .replace(/\n\s*\n/g, '\n')
        .replace(/\n/g, '\n    '),
    );
...
Page.prototype.insertTemporaryStyles = function(pageData) {
  ...
  $('dropdown').each((i, element) => {
    ...
    const placeholder = `<span class=${attributes.class}>${
      attributes.text
    }</span>`;
...
Page.prototype.generate = function(builtFiles) {
  ...
  return new Promise((resolve, reject) => {
    markbinder
      .includeFile(this.sourcePath, fileConfig)
      .then(result => {
      ...
      .then(result => {
        ...
        const newBaseUrl = calculateNewBaseUrl(
          this.sourcePath,
          this.rootPath,
          this.baseUrlMap,
        );
...
Page.prototype.preRender = function(content) {
  ...
  Object.entries(this.plugins).forEach(([pluginName, plugin]) => {
    if (plugin.preRender) {
      preRenderedContent = plugin.preRender(
        preRenderedContent,
        this.pluginsContext[pluginName] || {},
        this.frontMatter,
      );
    }
  });
...
Page.prototype.postRender = function(content) {
  ...
  Object.entries(this.plugins).forEach(([pluginName, plugin]) => {
    if (plugin.postRender) {
      postRenderedContent = plugin.postRender(
        postRenderedContent,
        this.pluginsContext[pluginName] || {},
        this.frontMatter,
      );
...
Page.prototype.collectPluginsAssets = function(content) {
  ...
  Object.entries(this.plugins).forEach(([pluginName, plugin]) => {
    if (plugin.getLinks) {
      const pluginLinks = plugin.getLinks(
        content,
        this.pluginsContext[pluginName],
        this.frontMatter,
        linkUtils,
    ...
    if (plugin.getScripts) {
      const pluginScripts = plugin.getScripts(
        content,
        this.pluginsContext[pluginName],
        this.frontMatter,
        scriptUtils,
...
Page.prototype.addLayoutFiles = function() {
  this.asset.layoutScript = path.join(
    this.layoutsAssetPath,
    this.frontMatter.layout,
    'scripts.js',
  );
...
Page.prototype.resolveDependency = function(dependency, builtFiles) {
  ...
  return new Promise((resolve, reject) => {
    const resultDir = path.dirname(
      path.resolve(this.resultPath, path.relative(this.sourcePath, file)),
    );
    const resultPath = path.join(
      resultDir,
      FsUtil.setExtension(path.basename(file), '._include_.html'),
    );
    ...
    if (FsUtil.isInRoot(this.rootPath, file)) {
      tempPath = path.join(
        path.dirname(this.tempPath),
        path.relative(this.rootPath, file),
      );
    } else {
      logger.info(
        `Converting dynamic external resource ${file} to ${resultPath}`,
      );
      tempPath = path.join(
        path.dirname(this.tempPath),
        '.external',
    ...
    return markbinder
      ...
      .then(result =>
        markbinder.resolveBaseUrl(result, {
          baseUrlMap: this.baseUrlMap,
          rootPath: this.rootPath,
          isDynamic: true,
      ...
      .then(() =>
        markbinder.renderFile(tempPath, {
          baseUrlMap: this.baseUrlMap,
          rootPath: this.rootPath,
        }),
      )
      .then(result => {
        ...
        return fs.outputFileAsync(
          resultPath,
          htmlBeautify(content, { indent_size: 2 }),
        );
      })
      .then(() => {
