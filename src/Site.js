...
function Site(
  ...
  onePagePath,
  forceReload = false,
  siteConfigPath = SITE_CONFIG_NAME,
) {
  ...
  this.siteAssetsDestPath = path.join(
    outputPath,
    TEMPLATE_SITE_ASSET_FOLDER_NAME,
  );

  // Page template path
  this.pageTemplatePath = path.join(
    __dirname,
    TEMPLATE_ROOT_FOLDER_NAME,
    PAGE_TEMPLATE_NAME,
  );
  this.pageTemplate = ejs.compile(
    fs.readFileSync(this.pageTemplatePath, 'utf8'),
  );
  this.pages = [];
...
function getIconsMap() {
  ...
  const fontAwesomeClasses = fs
    .readFileSync(fontAwesomePath, 'utf8')
    .trimRight()
    .split(/\r?\n/);
  const glyphiconsClasses = fs
    .readFileSync(glyphiconsPath, 'utf8')
    .trimRight()
    .split(/\r?\n/);
  const iconsMap = {};
...
Site.initSite = function(rootPath) {
  ...
  const siteDefaultLayoutScriptsPath = path.join(
    siteLayoutDefaultPath,
    LAYOUT_SCRIPTS_PATH,
  );
    ...
    fs.readJsonAsync(siteConfigPath)
      .then(config => {
        ...
        this.siteConfig.baseUrl =
          baseUrl === undefined ? this.siteConfig.baseUrl : baseUrl;
        this.siteConfig.enableSearch =
          config.enableSearch === undefined || config.enableSearch;
        resolve(this.siteConfig);
      })
      .catch(err => {
        reject(
          new Error(
            `Failed to read the site config file '${this.siteConfigPath}' at` +
              `${this.rootPath}:\n${
                err.message
...
Site.prototype.createPage = function(config) {
  ...
  const resultPath = path.join(
    this.outputPath,
    setExtension(config.pageSrc, '.html'),
  );
  return new Page({
    ...
    src: config.pageSrc,
    layoutsAssetPath: path.relative(
...
Site.prototype.updateAddressablePages = function() {
  ...
  return _.difference(
    oldAddressablePages.map(page => page.src),
...
Site.prototype.collectAddressablePages = function() {
  ...
  if (duplicatePages.length > 0) {
    return Promise.reject(
      new Error(
        `Duplicate page entries found in site config: ${_.uniq(
          duplicatePages,
        ).join(', ')}`,
      ),
    );
  }
  const globPaths = addressableGlobs.reduce(
    (globPages, addressableGlob) =>
      globPages.concat(
        walkSync(this.rootPath, {
          directories: false,
...
Site.prototype.generate = function(baseUrl) {
  ...
  logger.info(
    `Website generation started at ${startTime.toLocaleTimeString()}`,
  );
  return new Promise((resolve, reject) => {
    this.readSiteConfig(baseUrl)
      ...
      .then(() => {
        ...
        logger.info(
          `Website generation complete! Total build time: ${totalBuildTime}s`,
        );
      })
      .then(resolve)
      .catch(error => {
...
Site.prototype.rebuildAffectedSourceFiles = delay(
  Site.prototype._rebuildAffectedSourceFiles,
  1000,
);
...
Site.prototype.rebuildSourceFiles = delay(
  Site.prototype._rebuildSourceFiles,
  1000,
);
...
Site.prototype._buildMultipleAssets = function(filePaths) {
  ...
  const fileRelativePaths = uniquePaths.map(filePath =>
    path.relative(this.rootPath, filePath),
  );
  const copyAssets = fileIgnore
    .filter(fileRelativePaths)
    .map(asset =>
      fs.copyAsync(
        path.join(this.rootPath, asset),
        path.join(this.outputPath, asset),
      ),
    );
  return Promise.all(copyAssets).then(() => logger.info('Assets built'));
};
...
Site.prototype._removeMultipleAssets = function(filePaths) {
  ...
  const filesToRemove = fileRelativePaths.map(fileRelativePath =>
    path.join(this.outputPath, fileRelativePath),
  );
  const removeFiles = filesToRemove.map(asset => fs.removeAsync(asset));
  return Promise.all(removeFiles).then(() => logger.info('Assets removed'));
};
...
Site.prototype.generatePages = function() {
  ...
  if (this.onePagePath) {
    ...
    if (!page) {
      return Promise.reject(
        new Error(
          `${this.onePagePath} is not specified in the site configuration.`,
        ),
      );
    }
    this.pages.push(
    ...
    this.pages = addressablePages.map(page =>
      this.createPage({
        faviconUrl,
        pageSrc: page.src,
        title: page.title,
  ...
  const progressBar = new ProgressBar(
    `[:bar] :current / ${this.pages.length} pages built`,
    ...
    processingFiles.push(
      page
        ...
        .catch(err => {
          logger.error(err);
          return Promise.reject(
            new Error(`Error while generating ${page.sourcePath}`),
          );
        }),
    );
  });
...
Site.prototype.regenerateAffectedPages = function(filePaths) {
  const builtFiles = {};
  const processingFiles = [];
  const shouldRebuildAllPages =
    this.collectUserDefinedVariablesMapIfNeeded(filePaths) || this.forceReload;
  if (shouldRebuildAllPages) {
    logger.warn(
      'Rebuilding all pages as variables file was changed, or the --force-reload flag was set',
    );
  }
  this._setTimestampVariable();
  this.pages.forEach(page => {
    if (
      shouldRebuildAllPages ||
      filePaths.some(filePath => page.includedFiles[filePath])
    ) {
      // eslint-disable-next-line no-param-reassign
      page.userDefinedVariablesMap = this.userDefinedVariablesMap;
      processingFiles.push(
        page.generate(builtFiles).catch(err => {
          logger.error(err);
          return Promise.reject(
            new Error(`Error while generating ${page.sourcePath}`),
          );
  ...
  return new Promise((resolve, reject) => {
    Promise.all(processingFiles)
      .then(() =>
        this.updateSiteData(shouldRebuildAllPages ? undefined : filePaths),
      )
...
Site.prototype.copyLayouts = function() {
  ...
  const layoutsDestPath = path.join(
    this.siteAssetsDestPath,
  ...
  return new Promise((resolve, reject) => {
    ...
    const siteData = {
      ...
      pages: this.pages
        ...
        .map(page =>
          Object.assign({ headings: page.headings }, page.frontMatter),
        ),
    };
  ...
  return new Promise((resolve, reject) => {
    ...
    this.readSiteConfig()
      .then(() => {
        ...
        const options = {};
        options.branch =
          this.siteConfig.deploy.branch || defaultDeployConfig.branch;
        options.message =
          this.siteConfig.deploy.message || defaultDeployConfig.message;
        ...
        if (travisTokenVar) {
          ...
          // eslint-disable-next-line no-param-reassign
          travisTokenVar = _.isBoolean(travisTokenVar)
            ? 'GITHUB_TOKEN'
            : travisTokenVar;
          if (!process.env[travisTokenVar]) {
            reject(
              new Error(
                `The environment variable ${travisTokenVar} does not exist.`,
              ),
            );
            return undefined;
          }
