...
src | `String` | | The path to another local markdown `.md` or HTML `.html` file whose content will be loaded into the Panel body. <br> Supports file fragments by specifying `#fragment-id` at the end of the `src` attribute, provided the fragment is wrapped in a `<div>`/`<span>` tag with the matching id.
