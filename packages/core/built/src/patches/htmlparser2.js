...
var _a = require('htmlparser2'), Tokenizer = _a.Tokenizer, Parser = _a.Parser;
/*
...
/* eslint-disable
    ...
    vars-on-top,
*/
var i = 0, TEXT = i++, BEFORE_TAG_NAME = i++, //after <
IN_TAG_NAME = i++, IN_SELF_CLOSING_TAG = i++, BEFORE_CLOSING_TAG_NAME = i++, IN_CLOSING_TAG_NAME = i++, AFTER_CLOSING_TAG_NAME = i++, 
//attributes
...
//comments
BEFORE_COMMENT = i++, IN_COMMENT = i++, AFTER_COMMENT_1 = i++, AFTER_COMMENT_2 = i++, 
//cdata
BEFORE_CDATA_1 = i++, // [
BEFORE_CDATA_2 = i++, // C
BEFORE_CDATA_3 = i++, // D
...
HAS_MATCHING = -1, NO_MATCH = -2, HAS_MATCHED = -3;
var DEFAULT_SPECIAL_TAGS = [
    'script',
    'style',
    'variable',
];
...
Tokenizer.prototype.specialTagNames = __spreadArrays(DEFAULT_SPECIAL_TAGS);
/**
...
Tokenizer.prototype._matchSpecialTagsFirstCharacters = function (c) {
    ...
    var numSpecialTags = this.specialTagNames.length;
    var lowerCaseChar = c.toLowerCase();
    for (var j = 0; j < numSpecialTags; j += 1) {
...
Tokenizer.prototype._matchSpecialTagsNextCharacters = function (c) {
    var matchingSpecialTags = [];
    var numMatchingTags = this._matchingSpecialTagIndexes.length;
    var lowerCaseChar = c.toLowerCase();
    for (var j = 0; j < numMatchingTags; j += 1) {
        var tagIndex = this._matchingSpecialTagIndexes[j];
        var testChar = this.specialTagNames[tagIndex][this._nextSpecialTagMatchIndex];
...
Tokenizer.prototype._stateBeforeSpecial = function (c) {
    var result = this._matchSpecialTagsNextCharacters(c);
...
Tokenizer.prototype._matchNextSpecialTagClosingCharacter = function (c) {
    var nextTestChar = this.specialTagNames[this._special - 1][this._nextSpecialTagMatchIndex];
...
Tokenizer.prototype._stateBeforeSpecialEnd = function (c) {
    var result = this._matchNextSpecialTagClosingCharacter(c);
    if (result === HAS_MATCHING) {
        return;
    }
...
function injectIgnoreTags(tagsToIgnore) {
    Tokenizer.prototype.specialTagNames = __spreadArrays(DEFAULT_SPECIAL_TAGS, tagsToIgnore);
}
module.exports = {
    injectIgnoreTags: injectIgnoreTags,
};
