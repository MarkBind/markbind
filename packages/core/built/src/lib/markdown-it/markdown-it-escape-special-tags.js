...
function escape_plugin(md, tagsToIgnore) {
    ...
    var specialTagsRegex = Array.from(tagsToIgnore)
    ...
    var selfClosingNegativeLookahead = "(?!" + attribute + "*\\s*/>)";
    var startingSpecialTagRegex = new RegExp("^<(" + specialTagsRegex + ")" + selfClosingNegativeLookahead + "(?=(\\s|>|$))", 'i');
    var endingSpecialTagRegex = new RegExp("<\\/(" + specialTagsRegex + ")>", 'i');
    var HTML_SEQUENCES = [
