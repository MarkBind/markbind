...
async function requireFromString(src: string) {
  // Create temporary file to write vue bundle to, as ESM does not fully
  // support importing from strings without --experimental-vm-modules flag
  const __dirname = path.dirname(fileURLToPath(import.meta.url));
  const tmpFile = path.join(__dirname, `vue-module-${randomUUID()}.cjs`);

  try {
    await fs.writeFile(tmpFile, src);
  } catch (e) {
    logger.error(e);
    throw e;
  }
  logger.verbose(`Wrote vue-module bundle to: ${tmpFile}`);
