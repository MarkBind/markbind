...
/**
 * Interface for the service configuration used by the tokenizer.
 */
export interface EmbedService {
  extractVideoID: (reference: string) => string | null;
  [key: string]: any;
}

export interface EmbedServiceMap {
  [key: string]: EmbedService;
}
...
/**
 * Advances the pointer to the next occurrence of a specific symbol.
 */
function advanceToSymbol(state: StateBlock, endLine: number, symbol: string, pointer: Pointer): boolean {
  let symbolLine = pointer.line;
  let symbolIndex = state.src.indexOf(symbol, pointer.pos);

  if (symbolIndex === -1) return false;

  let maxPos = state.eMarks[pointer.line];
...
/**
 * Factory function to create the tokenizer with access to configured services.
 */
export function createTokenizer(services: EmbedServiceMap) {
  return function tokenizer(state: StateBlock, startLine: number, endLine: number, silent: boolean): boolean {
    let startPos = state.bMarks[startLine] + state.tShift[startLine];
    let maxPos = state.eMarks[startLine];

    let pointer: Pointer = { line: startLine, pos: startPos };
    ...
    let serviceName = state.src
      .substring(pointer.prevPos!, pointer.pos)
      .trim()
      .toLowerCase();
    ...
    // Read embed reference from within parenthesis: (reference)
    ...
    if (!silent) {
      const token = state.push('video', 'div', 0);
